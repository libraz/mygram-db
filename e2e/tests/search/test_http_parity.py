...
class TestHttpParity:
    ...
    def test_search_and_count_match_tcp(self, mysql, mygramdb, seed_data):
        ...
        tcp = mygramdb.search("articles", marker, sort="id ASC", limit=20)
        http = mygramdb.http_search("articles", marker, sort="id ASC", limit=20)
        assert http["status"] == 200
        assert http["total"] == tcp["total"]
        assert http["ids"] == tcp["ids"]

        tcp_filtered = mygramdb.search(
            "articles", marker, filters={"status": 1}, sort="id ASC", limit=20
        )
        ...
        http_count_status, http_count = mygramdb.http_count("articles", marker)
        assert http_count_status == 200
        assert http_count == mygramdb.count("articles", marker)
    ...
    def test_score_sort_matches_tcp(self, mysql, mygramdb, seed_data):
        ...
        tcp = mygramdb.search("articles", marker, sort="_score DESC", limit=10)
        http = mygramdb.http_search("articles", marker, sort="_score DESC", limit=10)
        assert http["status"] == 200
        assert http["total"] == tcp["total"]
        assert http["ids"] == tcp["ids"]

    def test_http_invalid_boolean_is_client_error(self, mygramdb, seed_data):
        status, body = mygramdb.http_post("/articles/search", {"q": "(broken OR", "limit": 10})
        assert status == 400
    ...
    def test_count_rejects_search_only_options(self, mygramdb, seed_data):
        status, body = mygramdb.http_post(
            "/articles/count", {"q": "test", "sort": {"column": "_score"}}
        )
        assert status == 400
        assert isinstance(body, dict)
        assert "error" in body
    ...
    def test_tcp_facet_has_e2e_coverage(self, mysql, mygramdb, seed_data):
        ...
        counts = mygramdb.facet("articles", "category", marker)
