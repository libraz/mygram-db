...
class TestCommandCounters:
    ...
    def test_search_counter(self, mygramdb, seed_data):
        ...
        search_metrics = {k: v for k, v in diff.items() if "search" in k.lower() or "command" in k.lower()}
        if search_metrics:
            max_increase = max(search_metrics.values())
            assert max_increase >= m, (
                f"Search counter increased by {max_increase}, expected >= {m}"
            )
    ...
    def test_count_counter(self, mygramdb, seed_data):
        ...
        count_metrics = {k: v for k, v in diff.items() if "count" in k.lower() or "command" in k.lower()}
        if count_metrics:
