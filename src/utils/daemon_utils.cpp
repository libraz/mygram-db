...
bool Daemonize() {
  ...
  if (pid < 0) {
    // Fork failed
    spdlog::error("Failed to fork process for daemonization");
    return false;
  }
  ...
  if (setsid() < 0) {
    spdlog::error("Failed to create new session with setsid()");
    return false;
  }
  ...
  if (pid < 0) {
    spdlog::error("Failed to fork process (second fork)");
    return false;
  }
