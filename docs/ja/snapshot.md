...
- MygramDBは起動時にダンプファイルを**自動読み込みしません**。再起動後に
  ダンプから復旧する場合は、MygramDBを起動し、`DUMP VERIFY`を実行してから
  `DUMP LOAD`を実行してください。
...
- 自動保存スナップショットはバックアップファイルであり、サーバー起動時に
  自動復元されるものではありません。
...
ダンプからの復旧は運用者が明示的に実行します。MygramDBは起動時に
`dump.dir`をスキャンしたり、最新ダンプを読み込んだりしません。まずサーバーを
起動し、対象ダンプを検証してから`DUMP LOAD`を実行してください。

`DUMP LOAD`が完了すると、MygramDBは自動的にレプリケーションを再開します：
